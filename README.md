...
| ~~`container-image-tags`~~       | Checks whether a local container image tag still matches its remote registry digest, then optionally finds every other remote tag pointing to that digest. **[Continued at [huyz/container-image-tags](https://github.com/huyz/container-image-tags)]** |
| `email-strip-inline-images`      | Removes inline images (not attachments) from email messages, replacing them with notes                                                                                                                                                            |
...
| `git-history-sizes`              | Lists files in git history and sort by size impact                                                                                                                                                                                                |
...
| `restic-snapshots-list`          | Lists restic snapshot IDs newer than a given date                                                                                                                                                                                                 |
...
| `sunshine`                       | Wrapper for Sunshine command that takes a display name to automatically configure output                                                                                                                                                          |
...
| `is-mac-addr-private`             | Checks if the Wi-Fi MAC address is private (e.g., randomized) or not                                                                                       |
